			...
			<div class="col-2">
				<div class="row">
					...
					<?php 
						...
						$join = mysqli_query($con, "SELECT * FROM users INNER JOIN videos ON users.id = videos.user_id WHERE users.id='{$_GET['id']}'");
					?>
					<div class="col-2 px-0">
						<!--аккаунт-->
						<a href="account.php?id=<?php echo $_GET['id']?>"><img src="img/7.jpg" alt="" class="w-75"></a>
					</div>
				</div>
			</div>
		</div>
					...
					<div class="col-3"> <!--ВИДЕО -->
						...
						<div class="row">
							...
							<div class="col-9">							
								...
								<p class="text-secondary"><?php echo $result['descr'] ?></p>
							</div>
						</div>						
					</div> <!--КОНЕЦ ВИДЕО -->		
				<?php } ?>
